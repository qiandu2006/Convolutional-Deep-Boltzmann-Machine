__kernel void pooling(__global const float *z0, __global const float *zp0, __global float *h, __global float *p)
{
    ...
    const int current = iz*gxsize*gysize+ix*gysize+iy;

    float ul = exp(z0[pUL]+zp0[current]);
    float ur = exp(z0[pUR]+zp0[current]);
    float dl = exp(z0[pDL]+zp0[current]);
    float dr = exp(z0[pDR]+zp0[current]);
